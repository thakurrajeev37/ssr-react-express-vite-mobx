  ...
  return async function ssrMiddleware(req, res, next) {
    try {
      ...
      if (!isProd) {
        ...
        renderModule = await import(path.resolve(process.cwd(), 'dist/server/entry-server.js'));
      }
      ...
      const { appHtml, stateScript = '', status = 200 } = await render(url, { initialStores });
      ...
      res.status(500).type('html').send('<!DOCTYPE html><html><head><title>500</title></head><body><h1>Internal Server Error</h1><p>SSR failure.</p></body></html>');
    }
  };
}
